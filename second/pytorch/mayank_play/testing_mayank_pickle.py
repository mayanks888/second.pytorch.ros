...
##########################################################3
    ...
    scan = np.fromfile('/home/mayank_sati/Documents/point_clouds/nucene_pickle/nuscene/n008-2018-05-21-11-06-59-0400__LIDAR_TOP__1526915243047392.pcd.bin', dtype=np.float32)
    ...
    for iIndex in range(iLen):
        ...
        RM = rotz(ry)
        bbox=np.concatenate((loc, dim,ry), axis=None)

        point_indices = box_np_ops.nuscenes_points_in_mayank(points, bbox, RM)
        gt_points = points[point_indices[:,0]]
        gt_points1 = points[point_indices[:,0]]
        gt_points[:, :3] -= loc[:]
        db_info = {
            "point": gt_points,
            "score": score,
        ...
        predictions_dicts.append(db_info)
        ##################################################################
    ...
    # with open(result_path_step / "apollo_pp.pkl", 'wb') as f:
    # with open("apollo_pp.pkl", 'wb') as f:
...
def evaluate_univ(config_path,
        ...
        print("Generate output labels...")
        example = example_convert_to_torch(example, float_dtype)
        dt_annos += predict_kitti_to_anno(net, example, class_names, center_limit_range, model_cfg.lidar_input,
                                          global_set)
        # with open(result_path_step / "result_pp1.pkl", 'wb') as f:
        #             pickle.dump(dt_annos, f)
        ...
        if anchor_area_threshold >= 0:
            ...
            dense_voxel_map2 = dense_voxel_map2.cumsum(0)
            dense_voxel_map2 = dense_voxel_map2.cumsum(1)
            anchors_area2 = box_np_ops.fused_get_anchors_area(dense_voxel_map2, anchors_bv, voxel_size, pc_range,
                                                              grid_size)
            anchors_mask2 = anchors_area2 > anchor_area_threshold
        kl2 = np.zeros((coordinates2.shape[0], 1))
        coordinates2 = np.append(kl2, coordinates2, axis=1)
