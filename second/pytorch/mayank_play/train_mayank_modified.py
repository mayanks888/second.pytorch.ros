...
def train(config_path,
    ...
    #target assigner is nothing but info about all the anchor box generated for all the class combined
    ...
    # PREPARE INPUT
    ######################
    dataset = input_reader_builder.build(input_cfg, model_cfg, training=True, voxel_generator=voxel_generator, target_assigner=target_assigner, multi_gpu=multi_gpu)
    eval_dataset = input_reader_builder.build(eval_input_cfg, model_cfg, training=False, voxel_generator=voxel_generator, target_assigner=target_assigner)

    dataloader = torch.utils.data.DataLoader(dataset, batch_size=input_cfg.batch_size * num_gpu, shuffle=True, num_workers=input_cfg.preprocess.num_workers * num_gpu, pin_memory=False, collate_fn=collate_fn, worker_init_fn=_worker_init_fn, drop_last=not multi_gpu)
...
def evaluate_mayank(config_path,
             model_dir=None,
             result_path=None,
    ...
    # # points = points.reshape((-1, 5))[:, :4]
    # voxels, coords, num_points = voxel_generator.generate(points, max_voxels=80000)
    #####################################################################
    ...
    eval_dataset = input_reader_builder.build(input_cfg, model_cfg, training=False, voxel_generator=voxel_generator, target_assigner=target_assigner)
    eval_dataloader = torch.utils.data.DataLoader(eval_dataset, batch_size=batch_size, shuffle=False, num_workers=input_cfg.preprocess.num_workers, pin_memory=False, collate_fn=merge_second_batch)
    ...
    for example in iter(eval_dataloader):
        ...
        with torch.no_grad():
            t7 = time.time()
            detections += net(example)
            print(net.get_avg_time_dict().items())
            print("prediction", (time.time() - t7)*1000)
        bar.print_bar()
        if measure_time:
            t2 = time.time()
        # break
...
if __name__ == '__main__':
    ...
    # config_path = '../configs/mayank/nus/all.fhd.config'
    # config_path = '../configs/mayank/xyres_16.config'
    # model_dir='../trained_dodel_dir'
    # model_dir = '../mayank_pc_trained'
    # model_dir = '../model/t59_allfhd'
    # result_path=
    ...
    config_path = '/home/mayank_sati/pycharm_projects/pytorch/second.pytorch_traveller59_date_9_05/second/configs/nuscenes/all.pp.largea_one_input.config'
    # model_dir = '/home/mayank_sati/pycharm_projects/pytorch/second.pytorch_traveller59_date_9_05/second/eval_result/pretrained_models_v1.5/pp_model_for_nuscenes_pretrain'
    model_dir = '/home/mayank_sati/pycharm_projects/pytorch/second.pytorch_traveller59_date_9_05/second/point_pp_nuscene_try'
    ###############################
    train(config_path, model_dir)
    evaluate_mayank(config_path, model_dir)
