    ...
    for filename in filenames:
        ...
        # try:
            ...
            for iIndex, box3d in enumerate(boxes_lidar, start=0):
                ...
                color = (0, 1, 0)
                line_width = 1
                clas=class_names[labels_lidar[iIndex]]
                # if clas=='pedestrian':
                #     clas='ped'
                mlab.points3d(x, y, z, color=(1, 1, 1), mode='sphere', scale_factor=0.4)
                mlab.text3d(x, y, z, clas, scale=(.6, .6, .6))
            ...
            # mlab.view(azimuth=270, elevation=30, focalpoint=[12.0909996, -1.04700089, -2.03249991], distance=120.0, figure=fig)
            mlab.view(azimuth=270, elevation=30, focalpoint=[ 0, 0, 0], distance=120.0, figure=fig)
            # mlab.view(azimuth=270, elevation=30, focalpoint='auto', distance=120.0, figure=fig)
            # mlab.view(45, 45)
            # mlab.view(240, 120)
            # mlab.view(distance=120)
            # mlab.view(focalpoint=(0, 0, 9))
            mlab.show()
            counter+=1
            print(counter)
    # cv2.imwrite('/home/mayank_sati/Desktop/pp/save_img/myimage_' + str(counter) + '.jpg', bev_map)
    #         mlab.savefig('/home/mayank_sati/Desktop/pp/save_lid_annotation/myimage_' + str(counter) + '.jpg', size=None, figure=None, magnification='auto', **kwargs)
    #         mlab.savefig('/home/mayank_sati/Desktop/pp/save_lid_annotation_without_score_sweep/myimage_' + str(counter) + '.jpg', size=None, figure=None, magnification='auto')
